...
def _xinetd_r_service_files():
    ...
    for filename in filenames:
        ...
        related = filename.lower() in R_SERVICE_NAMES
        disable_value = None
        ...
        try:
            with open(
                ...
                for line in file:
                    ...
                    if (
                        service_match
                        and service_match.group(1).lower()
                        in R_SERVICE_NAMES
                    ):
                        related = True
...
def _get_issues():
    ...
    inetd_lines, inetd_errors = _inetd_r_service_lines()

    for line_number in inetd_lines:
        issues.append(
            f"{INETD_PATH}:{line_number}: r 계열 서비스 활성 설정"
        )

    issues.extend(inetd_errors)

    xinetd_files, xinetd_errors = _xinetd_r_service_files()
    ...
    try:
        ...
        for line in lines:
            ...
            if (
                ...
                new_lines.append(f"# U-36 disabled: {line}")
                changed = True
            else:
                new_lines.append(line)
...
def check():
    ...
    if not issues:
        return result(
            ...
            TITLE,
            GOOD,
            "rlogin, rsh, rexec 등 r 계열 서비스가 비활성화되어 있습니다.",
        )
    ...
    if not remaining_issues:
        detail = (
            f"r 계열 서비스 활성 설정 확인 후 조치 완료: {before}. "
            f"관련 서비스를 중지하고 자동 시작 및 inetd/xinetd 설정을 "
            f"비활성화했습니다."
        )
